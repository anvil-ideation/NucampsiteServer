            ...
            .then(favorite => {
                if (favorite) {
                    if (favorite.campsite.includes(req.params.campsiteId) === false) {
                        if (db.collection('campsites').findOne({"_id": req.params.campsiteId}) === false) {
                            err = new Error(`Campsite ${req.params.campsiteId} does not exist`);
                            res.statusCode = 404;
                            return next(err);
                        } else {
                            console.log(`Campsite ${req.params.campsiteId} added to favorites`);
                        ...
                        err = new Error(`Campsite ${req.params.campsiteId} is already a favorite`);
                        res.statusCode = 404;
                        return next(err);
                    }
                } else {
                    ...
                    .then(favorite => {
                        ...
                        favorite.save()
                        .then(favorite => {
                            console.log(`Favorites document created and campsite ${req.params.campsiteId} added to favorites`);
                            res.statusCode = 200;
                            res.setHeader('Content-Type', 'application/json');
                            res.json(favorite);
                        })
    ...
    .then(favorite => {
        if (favorite) {
            if (favorite.campsite.includes(req.params.campsiteId) === true) {
                ...
                favorite.save()
                .then(favorite => {
                    console.log(`Campsite ${req.params.campsiteId} removed from favorites`);
                    res.statusCode = 200;
                    res.setHeader('Content-Type', 'application/json');
                    res.json(favorite);
                })
            } else {
                err = new Error(`Campsite ${req.params.campsiteId} is not a favorite`);
                res.statusCode = 404;
                return next(err);
            }
        } else {
